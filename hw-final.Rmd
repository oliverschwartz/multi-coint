...
We need to compute exponential rolling averages using a 1-hour window time. How do we compute an exponential rolling average? At a time $T$, the exponential average is given by the following update rule.
$$
EMA(t) = 
\begin{cases}
  p_t, &t = 0 \\
  \alpha \cdot p_t + (1-\alpha) \cdot EMA(t-1), & t > 0
\end{cases}
$$
