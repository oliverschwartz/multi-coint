...
author:
- Oliver Schwartz^[Princeton University, oliver.schwartz@princeton.edu]
- Daniel Gitelman^[Princeton University, daniel.gitelman@princeton.edu]
date: "5/5/2021"
output:
...
setwd("C:/Users/dangi/Dropbox/Classes/orf445/multi-coint")
...
Computing exponential correlations is a little more involved. Given the exponential average at some time $t=n$ for some instrument $x$, $EMA_x(n) = \frac{A_n}{B_n}$, the exponential covariance $Cov_{e}$ is simply the weighted mean from the beginning of time relative to the current exponential average:
$$
Cov_{e} = \sum_{j=1}^n e^{-(n-j)/m} \Big(x_j - EMA_x(n)\Big)\Big(y_j - EMA_y(n)\Big)
$$
Similarly, the exponential standard deviation would be: 
$$
\sigma_e = \sqrt{\sum_{j=1}^n e^{-(n-j)/m} \Big(x_j - EMA_x(n)\Big)^2}
$$
...
svals <- matrix(data=NA, nrow=NSEC, ncol=NSYM)
...
```
Let's populate missing signal values with the previous non-missing value, and calculate the singular values by taking the square root of ordered eigenvalues. 
```{r}
signals <- na.locf(signals, na.rm=FALSE)
svals <- sqrt(svals)
svals <- na.locf(svals, na.rm=FALSE)
```

We now plot the evolution of the correlation matrix singular values over the course of the trading day.
```{r}
plot(svals[,1],type="l",ylim=c(0,2.5),xlab='Time', ylab='Singular Value',
     main='Evolution of Correlation Matrix Singular Values',col='orange')
for (i in 2:NSYM) {
  lines(svals[,i])
}
legend(41000, 1.85, legend=c("1st Sing. Val", "2nd-8th Sing. Vals"),
       col=c("orange", "black"), lty=1, cex=0.8)
```

We can observe from the above that the first singular value is distinctly larger than the remaining seven singular values, indicating that perhaps only the first transformed dimension is a random walk while the remaining are mean-reverting. This motivates the setting of transformed dimensions 2-8 to 0 in the previous calculation of the signal.
...
```
We can observe rather strong signals across the different Eurodollar maturities, with an increase in gradient values the closer the maturity is. This can likely be a result of higher liquidity in these symbols, with larger trading volume allowing for quicker reversions to the "expected" price as per the signal.
...
plot_strength_by_lag(NSYM)
```

For all instruments, we can observe that the predictable price motion increases with larger lags, indicating that prices are more likely to mean revert as we allow for more time to do so. Again, symbols with nearer maturities appear to demonstrate higher predictable price motions.