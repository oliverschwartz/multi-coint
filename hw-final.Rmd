...
rolling_stats <- function(x, y, wlen) {
  ...
  ema_y <- c(y[1])    
  rectmean_x <- c(x[1]) # Rectangular mean (for below two quantities). 
  rectmean_y <- c(y[1])   
  ...
  for (t in 2:N) {
    ...
    if (t > wlen) {
      rectmean_x[t] <- (x[t] + wlen*rectmean_x[t-1] - x[t-wlen]) / wlen
      rectmean_y[t] <- (y[t] + wlen*rectmean_y[t-1] - y[t-wlen]) / wlen
      sqr_x[t] <- (x[t]**2 + wlen*sqr_x[t-1] - x[t-wlen]**2) / wlen
      sqr_y[t] <- (y[t]**2 + wlen*sqr_y[t-1] - y[t-wlen]**2) / wlen
      prcx_prcy[t] <- (
        x[t]*y[t] + wlen*prcx_prcy[t-1] - x[t-wlen]*y[t-wlen]
        ) / wlen
    } 
    
    # We haven't seen a window's worth of data (only t-1 prior values). 
    else { 
      rectmean_x[t] <- (x[t] + (t-1)*rectmean_x[t-1]) / t
      rectmean_y[t] <- (y[t] + (t-1)*rectmean_y[t-1]) / t
      sqr_x[t] <- (x[t]**2 + (t-1)*sqr_x[t-1]) / t
      sqr_y[t] <- (y[t]**2 + (t-1)*sqr_y[t-1]) / t
      prcx_prcy[t] <- (x[t]*y[t] + (t-1)*prcx_prcy[t-1]) / t
    }
    
    # Compute standard deviation and correlation. 
    std_x[t] <- sqrt(sqr_x[t] - rectmean_x[t]**2)
    std_y[t] <- sqrt(sqr_y[t] - rectmean_y[t]**2)
    corr[t] <- (prcx_prcy[t] - rectmean_x[t]*rectmean_y[t]) / (std_x[t]*std_y[t])
  }
  ...
  return(list(
    ...
    ema_x=ema_x, 
    ema_y=ema_y, 
    rectmean_x=rectmean_x,
    rectmean_y=rectmean_y
  ))
}

rolling_stats(c(1,2,3), c(1,2,3), wlen=2)
```
...
for (i in 1:(length(SYMS)-1)) {
  for (j in (i+1):length(SYMS)) {
    ...
    pairwise_stats[[key]] <- rolling_stats(
      x=mid_by_sym[[SYMS[i]]]$mid[1:100],
      y=mid_by_sym[[SYMS[j]]]$mid[1:100],
      wlen=WINDOW_LEN
    )
    break
  }
